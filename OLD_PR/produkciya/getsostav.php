...
height:80px;
    ...
    <nav  aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item" ><a href="/produkciya">Номенклатура</a></li>
...
$prd=$_GET['naimenprodukt'];
$sos=mysqli_query($dbm,"select * FROM SOSTAV where ID_PRODUKCIYA='".$prd."' ORDER BY PRODUKT");

echo '<h2>Состав: <span class="badge badge-info">'.strtoupper($prd).'</span></h2>';
echo '<h5 class="mr-sm-2">Цена:<span class="badge badge-info">'.number_format($_GET['sebest'],3,'.',' ')."р.</span></h5>";
echo "<script>";
echo '$nacenka="'.$_GET['nacenka'].'";';
echo '$sebest="'.$_GET['sebest'].'";';
echo "</script>";
...
echo '<tr>';
echo '<th>Наименование</th><th>Ед.</th><th>Кол-во</th><th>Цена за ед.</th><th>Сумма</th><th>Редактировать</th><th>Удалить</th>';
echo '</tr>';
while ($Row=mysqli_fetch_assoc($sos))
{
    $proverkasklad=mysqli_query($dbm,"SELECT PRODUKT FROM SKLAD WHERE PRODUKT='".$Row['PRODUKT']."' LIMIT 1");
    $prsk=mysqli_fetch_row($proverkasklad);
echo '<tr>';
echo '<td>';
 echo $Row['PRODUKT'];
    if ($prsk[0]!==$Row['PRODUKT'])
    {
        echo " <span class=\"badge badge-danger\"> Нет на складе. Списание не возможно.</span>";
    }
echo '</td>';
echo '<td>';
...
echo '</td>';
    echo '<td>';
    echo number_format($Row['CENA'],3,'.',' ').'p.';
    echo '</td>';
echo '<td>';
/*$sosi=mysqli_query($dbm,"SELECT MAX(CENA) AS CENA, ID FROM SKLAD WHERE PRODUKT='".$Row['PRODUKT']."' ORDER BY ID");
...
while ($sebestoimost=mysqli_fetch_assoc($Qseb)){
	...
	$nacenka_pr=number_format((($_GET['sebest']-$sebestoimost['SUMMA'])/$sebestoimost['SUMMA'])*100,1,'.',' ');
	}
	if (isset($nacenka_pr)){
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Выбрать </h4>
...
echo "</select><br>";
echo "Коичество:";
echo "<input type='number' class='form-control' size='10' id='kolprod' value='0'>";
